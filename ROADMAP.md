...
- **Auto-detect project root** — no `WATCH_PATHS` config needed; server resolves root from `CLAUDE_PROJECT_DIR` (auto-injected by Claude Code) or `PROJECT_ROOT` (set by `continuum init`), falling back to walking up from `process.cwd()` for a marker file, per-project DB in `.continuum/knowledge.db`. (Corrected 2026-07-09: an earlier version of `continuum init` wrote an `.mcp.json` `"cwd"` field, which is not a supported field in Claude Code's schema and was silently ignored — fixed to use the documented `CLAUDE_PROJECT_DIR` mechanism instead.)
